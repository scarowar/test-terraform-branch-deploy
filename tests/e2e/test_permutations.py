...
COMMAND_VARIATIONS: list[CommandVariation] = [
    ...
    CommandVariation(
        ...
        expect_success=False,  # TF fails because module.usecase doesn't exist - tests parsing only
...
class TestExtraArgsSyntax:
    ...
    def test_target_with_quoted_key(self, runner: E2ETestRunner) -> None:
        ...
        # Note: module.test["key"] doesn't exist, but the parsing is tested
        run, _ = runner.run_command_test(
            test_name="args_quoted_key",
            command='.plan to dev | -target=module.test["key"]',
            expect_success=False,  # TF fails (no module.test) but parsing is tested
        )
        assert run.is_complete
