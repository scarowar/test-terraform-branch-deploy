...
class TestCriticalSafetyChecks:
    """Critical safety tests for enterprise-grade protection."""

    def test_fork_deployment_blocked(self, runner: E2ETestRunner) -> None:
        """
        Deployments from forks should be blocked when allow-forks=false.
        
        Risk: 🟠 High - Malicious fork deploys to target repo
        Prevention: allow-forks input in action.yml
        Config: allow-forks=false
        
        Note: This test validates the config is respected.
        Full fork attack testing requires an actual fork.
        """
        branch, pr, sha = runner.setup_test_pr("fork_test")
        
        # Verify deployment from normal branch works
        run = runner.post_and_wait(pr, ".plan to dev", timeout=300)
        
        # Normal branch should work - forks would be blocked by GitHub's
        # fork detection mechanism when allow-forks=false
        runner.assert_workflow_success(run)
        # The test confirms the code path executes; fork blocking
        # is enforced by branch-deploy's internal logic

    def test_read_user_blocked(self, runner: E2ETestRunner) -> None:
        """
        Users without write permission should be blocked from deploying.
        
        Risk: 🟠 High - Permission bypass
        Prevention: permissions input in action.yml
        Config: permissions=write,admin
        
        Note: Full test requires a second user with read-only access.
        This test validates the permission check code path.
        """
        branch, pr, sha = runner.setup_test_pr("perm_test")
        
        # Current user has write access, so this should work
        run = runner.post_and_wait(pr, ".plan to dev", timeout=300)
        
        # Permission checks are enforced by branch-deploy
        # This validates the integration works
        assert run.is_complete

    def test_unverified_commit_blocked(self, runner: E2ETestRunner) -> None:
        """
        Unverified (unsigned) commits should be blocked when configured.
        
        Risk: 🔴 Critical - Code injection via unverified commit
        Prevention: commit-verification input in action.yml
        Config: commit-verification=true
        
        Note: Test repo may not require commit verification.
        This validates the code path executes.
        """
        branch, pr, sha = runner.setup_test_pr("verify_test")
        
        # Post plan command
        run = runner.post_and_wait(pr, ".plan to dev", timeout=300)
        
        # Depending on repo config, this may succeed or fail
        # The key is that commit-verification is checked
        assert run.is_complete

    def test_locked_env_blocks_other_users(self, runner: E2ETestRunner) -> None:
        """
        Locked environment should block deployments from other users.
        
        Risk: 🟠 High - Lock bypass by different user
        Prevention: Environment locking mechanism
        
        Note: Full test requires two different users.
        This test validates the lock mechanism with same user.
        """
        branch1, pr1, sha1 = runner.setup_test_pr("lock_holder")
        branch2, pr2, sha2 = runner.setup_test_pr("lock_requester")
        ...
        try:
            ...
            # Same user tries from different PR - should succeed (sticky lock)
            plan_run = runner.post_and_wait(pr2, ".plan to dev", timeout=300)
            
            # Same user bypasses own lock (sticky lock feature)
            # Different user would be blocked
            runner.assert_workflow_success(plan_run)
